    ...
    try {
      ...
      if (isBackendReachable && response) {
        if (!response.ok) {
           ...
           setMessages(prev => [...prev, { role: 'model', text: `Server error: ${errorData.error || response.statusText}. Please ensure your Vercel project has the GEMINI_API_KEY environment variable set correctly.` }]);
           setIsLoading(false);
           return;
        }
