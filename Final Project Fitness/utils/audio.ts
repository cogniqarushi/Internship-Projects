...
export function downsampleBuffer(buffer: Float32Array, inputRate: number, outputRate: number): Float32Array {
  ...
  if (inputRate < outputRate) {
    // Upsampling logic could go here, but usually not needed for mic input
    // For simplicity, just return buffer or throw error
    return buffer; 
  }
...
export function createPcmBlob(data: Float32Array): { data: string; mimeType: string } {
  ...
  for (let i = 0; i < l; i++) {
    // Clamp values to range [-1, 1] before converting
